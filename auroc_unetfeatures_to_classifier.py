...
model_loaded = torch.load("Unet model here", map_location=lambda storage, loc: storage)
...
model_loaded = torch.load("Classifier on Unet model here", map_location=lambda storage, loc: storage)
...
for batch_idx, (data_now, target_now, filenames_current) in enumerate(data_loaders_test):
    ...
    target_output_model = model_trained(features_output_model)
    if batch_idx == 0:
