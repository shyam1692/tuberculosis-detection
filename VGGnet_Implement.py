...
"""
Right now, we want to write code for model creation of VGG Net
"""

#model = models.vgg16(pretrained=True)
model_to_train = models.vgg16(pretrained=False)
model_to_train.load_state_dict(torch.load("./vgg16-397923af.pth"))

params_model_to_train = np.array(list(model_to_train.parameters()))
#params_model = np.array(list(model.parameters()))

#Both are same
params_model_to_train[0].shape
#params_model[0].shape
...
#Abhialshs model
"""
Abhilash's model to be used temp
"""
...
vgg16 = vgg16.load_state_dict(torch.load("./snapshots/epoch-5-trainerror-0.1447084233261339-testerror-0.15577889447236182.pth"))

model_now = torch.load('my_file.pt', map_location=lambda storage, location: 'cpu')
torch.load('my_file.pt', map_location=lambda storage, loc: storage)

model_loaded = torch.load("./snapshots/epoch-5-trainerror-0.1447084233261339-testerror-0.15577889447236182.pth", map_location=lambda storage, loc: storage)
model_loaded2 = torch.load("./snapshots/epoch-1-trainerror-0.3002159827213823-testerror-0.18090452261306533.pth", map_location=lambda storage, loc: storage)


model_loaded
model_loaded.keys()
model_loaded['classifier.6.bias']
model_loaded2['classifier.6.bias']
vgg16.state_dict()['classifier.6.bias']

vgg16.load_state_dict(model_loaded2)
model_now = vgg16.load_state_dict(model_loaded)

type(model_now)
model_now.classifier
vgg16.classifier
vgg16.state_dict()
